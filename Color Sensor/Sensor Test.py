...
def readAllLoop():
    ...
    # w = getWhite()
    r = map(getRed(), 0, 4124, 0, 254)
    g = map(getGreen(), 0, 4124, 0, 254)
    b = map(getBlue(), 0, 4124, 0, 254)  # just a little balancing act
    w = map(getWhite(), 0, 4124, 0, 254)
    kMax = r
    klr = "red"
    if (g > kMax):
        kMax = g
    ...
    loopTimer = threading.Timer(0.5, readAllLoop)
    loopTimer.start()
